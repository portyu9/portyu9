...
def validate_spotlight_pr_response_evidence(
    ...
    for label, job in (("proposer", propose), ("approval", approve)):
        for fragment in schema_fragments:
            require(
                fragment in job,
                f"Spotlight {label} PR response schema changed: {fragment}",
            )
    ...
    require(
        ...
        < propose.index(reviewer_mutation)
        < propose.index(reviewer_schema)
    ...
    require(
        approve.index(approval_fetch)
        < approve.index(approval_schema)
        < approve.index(approval_consume)
        < approve.index(post_review_fetch)
        < approve.index(post_review_schema)
        < approve.index(post_review_consume),
    ...
    require(
        ...
        and spotlight.count('validate_spotlight_open_pr_object "    legacy = project_spotlight_privileged_refs_to_legacy(
...
if __name__ == "__main__":
    ...
    if run_self_test:
        weakened = spotlight.replace(
            post_review_schema,
            'true # adversarially removed post-review PR schema',
            1,
        )
        try:
            ...
            raise ValueError(
                "Spotlight PR response self-test accepted an untyped post-review snapshot"
            )

        current = '(.requested_reviewers | type == "array" and length <= 100) and'
        replacement = '(.requested_reviewers | tostring | length <= 100) and'
        require(
            propose.count(current) == 1,
            "Spotlight proposer reviewer-array self-test anchor changed",
        )
        propose_start = spotlight.index("  propose:\n")
        propose_end = spotlight.index("  approve:\n", propose_start)
        weakened_propose = propose.replace(current, replacement, 1)
        weakened = (
        ...
        try:
            ...
            raise ValueError(
                "Spotlight PR response self-test accepted type-coercing reviewer evidence"
            )
