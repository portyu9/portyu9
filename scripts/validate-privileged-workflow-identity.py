...
if __name__ == "__main__":
 ...
 + variable + f'" "{expected_ref}" "${expected_sha}"'
            )
            if variable == "CURRENT_MAIN_REF_RESPONSE":
                consume = 'CURRENT_MAIN_SHA="$(jq -r .object.sha <<<"$CURRENT_MAIN_REF_RESPONSE")"'
            else:
                consume = (
                    'test "$(jq -r .object.sha <<<"    created_ref = "python3 scripts/codeql_autofix_controller.py created-ref-response"
    reviewer = "python3 scripts/codeql_autofix_controller.py reviewer-request-response"
    require(autofix.count(created_ref) == 1,
            "CodeQL Autofix created-ref response validator identity changed")
    require(autofix.count(reviewer) == 1,
...
if __name__ == "__main__":
 ...
 + variable + f'")" = "${expected_sha}"'
            ...
            require(
                fetch_pos < validate_pos < consume_pos,
                f"Spotlight {job} Git-ref evidence must validate before SHA consumption: {expected_ref}",
            )
            cursor = consume_pos
