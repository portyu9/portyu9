...
def validate_bounded_observation(policy: dict[str, Any], texts: dict[str, str]) -> None:
    ...
    require(len(declared) == 12, "retry policy must classify exactly the current 12 bounded seq loops")
    ids = [item.get("id") for item in declared if isinstance(item, dict)]
    require(len(ids) == len(set(ids)) == 12 and all(isinstance(value, str) and value for value in ids),
...
if __name__ == "__main__":
    ...
    print(
        ...
        "unclassified generator/ruleset failures are terminal; all 12 bounded seq loops are declared "
