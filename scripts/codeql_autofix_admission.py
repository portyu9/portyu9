...
API responses and its own controller receipt, never from PR title/body/labels. This module
...
# These files/prefixes are either trusted-control source, governance validators, or can alter
# the repository's security/automation contract. Autofix PRs may propose changes here, but
# they must go through the ordinary prior-authorization/review path rather than auto-merge.
...
def check(name: str, value: Any, *, head: str) -> dict[str, Any]:
    return {
        "name": name,
        "status": "completed",
        "conclusion": "success",
...
def fixture() -> dict[str, Any]:
    ...
    return {
        ...
        "draft": False,
        "mergeable": True,
        "requiredChecks": [check(name, None, head=head) for name in REQUIRED_CHECKS],
        "security": {
            "headSha": head,
...
def main() -> int:
    self_test()
    print(
        "CodeQL Autofix admission self-test passed: only exact-head, single-alert, ordinary Python source fixes "
        "with trusted provenance, six expected green checks, no unresolved reviews, and a clean Advanced Security proof "
        "are eligible for protected auto-merge."
    )
    return 0
