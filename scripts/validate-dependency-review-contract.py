    ...
    steps:
      ...
      - name: Bind exact dispatched candidate
        ...
        run: |
          ...
          [[ "$HEAD_SHA" =~ ^[0-9a-f]{{40}}$ ]]
          [[ "$HEAD_REF" =~ ^refs/heads/dependabot/github_actions/[A-Za-z0-9_.-]+(/[A-Za-z0-9_.-]+)*$ ]]
      ...
      - name: Review dependency changes
        ...
        with:
          base-ref: ${{{{ steps.exact.outputs.base_sha || github.event.pull_request.base.sha }}}}
          head-ref: ${{{{ steps.exact.outputs.head_sha || github.event.pull_request.head.sha }}}}
...
def validate_workflow(text: str) -> None:
    ...
    for fragment in (
        ...
        "HEAD_SHA: ${{ github.sha }}",
        "HEAD_REF: ${{ github.ref }}",
        '[[ "$HEAD_REF" =~ ^refs/heads/dependabot/github_actions/',
        'test "$(git rev-parse HEAD)" = "$HEAD_SHA"',
        'BASE_SHA="$(gh api "repos/${TARGET_REPOSITORY}/git/ref/heads/main" --jq .object.sha)"',
        "printf 'base_sha=%s\\n'",
        "printf 'head_sha=%s\\n'",
    ...
    for forbidden_permission in (
        ...
        "security-events: write", "id-token: write", "attestations: write", "packages: write",
    ):
...
def validate_governance(text: str) -> None:
    for phrase in (
        ...
        "Dependency review / dependency-review", "moderate-or-higher",
        "runtime, development, and unknown", "license policy", "contents: read",
        "no path filters", "exact-head controller dispatch",
    ):
...
def self_test() -> None:
    ...
    mutations = (
        (EXPECTED_WORKFLOW.replace("fail-on-severity: moderate", "fail-on-severity: high"), "moderate-or-higher"),
        (EXPECTED_WORKFLOW.replace("warn-only: false", "warn-only: true"), "fail rather than warn"),
        (EXPECTED_WORKFLOW.replace("      contents: read", "      contents: write"), "forbidden authority"),
        (EXPECTED_WORKFLOW.replace("  pull_request:\n", "  pull_request:\n    paths:\n      - '.github/**'\n"), "path filters"),
        (EXPECTED_WORKFLOW.replace('test "$(git rev-parse HEAD)" = "$HEAD_SHA"', "true"), "dispatch binding"),
        (EXPECTED_WORKFLOW.replace('[[ "$HEAD_REF" =~ ^refs/heads/dependabot/github_actions/', '[[ "$HEAD_REF" =~ ^refs/heads/'), "dispatch binding"),
        (EXPECTED_WORKFLOW.replace("comment-summary-in-pr: never", "comment-summary-in-pr: always"), "must not require pull-request write permission"),
        (EXPECTED_WORKFLOW.replace(DEPENDENCY_REVIEW_SHA, "v5"), f"reviewed {DEPENDENCY_REVIEW_RELEASE} commit SHA"),
    )
...
def main() -> int:
    try:
        ...
        print(
            "Dependency Review governance validation passed: every PR plus the exact native-Dependabot dispatched head is checked "
            "read-only against live current main; moderate-or-higher vulnerabilities across every dependency scope fail closed and "
            "license policy stays isolated."
        )
        return 0
