...
EXPECTED_CONTEXTS = frozenset({*LEGACY_CONTEXTS, "trusted-capability-admission"})
...
def self_test(payload: dict[str, Any]) -> None:
    ...
    try:
        ...
        require("every reviewed migration state" in str(exc), f"migration status self-test failed for wrong reason: {exc}")
    else:
