...
def build_state(env: dict[str, str]) -> dict[str, Any]:
    ...
    require(env_value(env, "GITHUB_RUN_ID", POSITIVE), "Profile Stats run ID is missing")
    require(env_value(env, "GITHUB_RUN_ATTEMPT", POSITIVE), "Profile Stats run attempt is missing")
...
def main() -> int:
    ...
    try:
        ...
        state = build_state(dict(os.environ))
