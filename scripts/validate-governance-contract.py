...
Signal Field generation/publishing, fresh-run concurrency, and artifact-only publish
behavior. The companion .github/GOVERNANCE.md records the settings-level controls that
must mirror these checks in GitHub.
"""
...
def validate_stats(text: str) -> None:
    ...
    require(text.count("runs-on: ubuntu-24.04") == 2, "Both stats jobs must pin ubuntu-24.04")

    generate = job_block(text, "generate", "publish")
    ...
    require("contents: write" not in generate, "Third-party generation job received write authority")
...
def main() -> int:
    try:
        ...
        print(
            "Repository governance validation passed: PR checks are stable and read-only, pinned-upstream "
            "integration is mandatory in workflow code, third-party generation has no write authority, "
            "publication is isolated/revalidated, and the settings-level protection contract is documented."
        )
        return 0
