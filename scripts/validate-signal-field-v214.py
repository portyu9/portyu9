#!/usr/bin/env python3
"""Validate final Signal Field v2.14 Evidence ID semantics across all four variants."""
...
def load_identifier():
    spec = importlib.util.spec_from_file_location("signal_field_evidence_id", IDENTIFIER_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError("Unable to load Signal Field Evidence ID implementation")
...
def validate_directory(directory: Path) -> tuple[str, str]:
    ...
    print(
        "Signal Field v2.14 Evidence ID validation passed: four variants share one deterministic "
        f"semantic identity {evidence_id} and full {digest}."
    )
...
def self_test() -> None:
    ...
    print("Signal Field v2.14 validator self-test passed")
