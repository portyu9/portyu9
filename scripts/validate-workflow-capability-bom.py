...
def validate_snapshot() -> tuple[int, int]:
    ...
    require(len(workflows) == 11, f"Workflow Capability BOM workflow count changed: {len(workflows)}")
    require(jobs > 0, "Workflow Capability BOM contains no jobs")
    return len(workflows), jobs


def trusted_diagnostic() -> None:
    """Disposable carrier: evaluate the frozen #650 candidate with exact accepted-main admission code."""
    if os.environ.get("GITHUB_ACTIONS") != "true":
        return
    base_sha = "b482f450b35cb4bc0ba6250696038e5873705da8"
