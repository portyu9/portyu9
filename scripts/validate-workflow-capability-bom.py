...
def validate_snapshot() -> tuple[int, int]:
    ...
    require(len(workflows) == 9, f"Workflow Capability BOM workflow count changed: {len(workflows)}")
    require(jobs > 0, "Workflow Capability BOM contains no jobs")
    return len(workflows), jobs


def trusted_diagnostic() -> None:
    """Disposable review-carrier diagnostic: reproduce trusted-main rejection and print only its exact tuple."""
    if os.environ.get("GITHUB_ACTIONS") != "true":
        return
    base_sha = "5f244f8163ab79c26fcd212035386dae6816a4ec"
    ...
    with tempfile.TemporaryDirectory(prefix="trusted-base-") as temporary:
        ...
        candidate_tree_sha = subprocess.check_output(["git", "rev-parse", "HEAD^{tree}"], cwd=root, text=True).strip()
        code = (
            "from pathlib import Path\n"
            "import sys\n"
