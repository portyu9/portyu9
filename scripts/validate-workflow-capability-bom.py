...
def validate_snapshot() -> tuple[int, int]:
    ...
    require(len(workflows) == 12, f"Workflow Capability BOM workflow count changed: {len(workflows)}")
    require(jobs > 0, "Workflow Capability BOM contains no jobs")
    return len(workflows), jobs


def trusted_diagnostic() -> None:
    """Disposable carrier: evaluate frozen #723 with exact accepted-main admission code."""
    if os.environ.get("GITHUB_ACTIONS") != "true":
        return

    base_sha = "ecd4757c45ee57ac41b0b725e7ce184560025a65"
