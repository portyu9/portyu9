...
| Spotlight / `merge-readme-only-terminal-write` | `contents: write`, `pull-requests: read`, `checks: read`, `attestations: read` | independently revalidate the exact transaction, cryptographically verify the certificate, then perform only the expected-head README merge and exact candidate cleanup |
...
- `merge-readme-only-terminal-write` intentionally holds `contents: write` plus attestation-read authority; safety depends on independent live PR/ref/check revalidation, exact certificate/subject equality, constrained cryptographic attestation verification, final root/ref freshness checks, expected-head merge semantics, and the server-side no-bypass Main ruleset.
