...
17. a Spotlight terminal merge requires a deterministic attested Merge Authorization Certificate produced after independent live revalidation, signed by a separately lease-bound OIDC authority, and cryptographically verified by the terminal writer without granting that writer signing authority.
...
| Spotlight Merge Authorization Certificate | deterministic short-lived authorization receipt binding the exact candidate/PR/source/run/check transaction; terminal freshness is independently re-proved |
...
| GitHub Actions token capabilities | separated read, Actions-read lease/admission, reductive cleanup, Actions-mutation, PR/content mutation, signing, attestation verification, and publication authority |
...
Read-only is the default. Autonomous transaction workflows mint short-lived leases before write-capable jobs. Spotlight planning, lease minting, reductive reconciliation, source-epoch admission, quarantine, proposal, approval, read-only merge-authorization preparation, isolated certificate signing, and terminal certificate-verifying merge are explicit separate jobs so the authority graph itself is inspectable:
...
| Spotlight / `approve-bot-pr-checks-only` | `contents: read`, `actions: write`, `pull-requests: write` | approve only exact canonical required workflow runs after fail-closed lease/source/head/file checks, de-duplicate approval requests, and post one idempotent exact-head automation-approval audit comment |
| Spotlight / `prepare-merge-authorization-read-only` | `contents: read`, `pull-requests: read`, `checks: read`, `actions: read` | independently re-prove live roots, candidate/PR topology, the three canonical workflow runs, and exact five checks before deterministic certificate construction |
| Spotlight / `attest-merge-authorization-write-only` | `contents: read`, `id-token: write`, `attestations: write` | under the exact lease, digest-check and attest only the deterministic Merge Authorization Certificate subject with the canonical pinned signer |
| Spotlight / `merge-readme-only-terminal-write` | `actions: read`, `contents: write`, `pull-requests: read`, `checks: read`, `attestations: read` | independently revalidate the exact transaction and live trusted Capability Admission run/check proof, cryptographically verify the certificate, then perform only the expected-head README merge and exact candidate cleanup |
...
The Spotlight graph similarly prevents authority collapse. `reconcile-stale-candidates-write` has no checkout/Python, no Actions/check authority, and a closed monotone-reductive API surface: after the exact lease plus namespace, bot identity, one-parent/one-README topology, PR identity, and stale-age proofs it may close one stale bot PR and delete its exact stale ref. `mutation-budget-read-only` can observe only Actions history; it cannot mutate repository content, PRs, or workflow runs. `mutation-budget-quarantine-read-only` can only report and fail. `prepare-merge-authorization-read-only` can only observe repository/PR/check/Actions state and construct the deterministic certificate; it cannot mutate or sign. `attest-merge-authorization-write-only` has signing authority but no content-write, PR, check, or Actions-write authority. The terminal merger can write repository contents through the expected-head merge endpoint and read PR/check/attestation metadata, but it cannot approve workflow runs or mint attestations.

### 5. Shell/data boundary

GitHub expressions are forbidden inside `run:` shell source. Dynamic values must cross reviewed non-shell fields such as `env:`, `with:`, or `if:`. Token-authorized shell jobs—including the reconciler and certificate signer—additionally reject command-name aliases, indirect execution wrappers, alternate runner-resident interpreters/network clients, and unauthorized Git execution. The terminal merge permits exactly one separately reviewed literal `gh attestation verify` boundary; arbitrary non-API GitHub CLI execution remains forbidden.
...
Every accepted `main` push wakes Spotlight reconciliation immediately; Profile Stats dispatch remains the publication-coupled trigger and the hourly schedule is fallback-only. `mint-mutation-lease-read-only` follows planning with `actions: read` only. It binds the exact Spotlight run/attempt/workflow, planned `main` source, and deterministic `(main SHA, generated SHA, proposed README digest)` candidate to a 30-minute capability. Reconciliation, proposal, approval, certificate signing, and merge each independently prove that lease and sufficient minimum remaining lifetime for their full hard timeout.
...
After exact lease proof, successful reconciliation, and positive admission, `propose-readme-only-write` may create or reuse only the exact content-addressed immutable candidate branch and its README-only PR. `approve-bot-pr-checks-only` re-proves the exact lease, revalidates both source roots and the one-commit README-only topology, binds the three canonical default-branch workflow identities, locally de-duplicates each `/approve` mutation while waiting for success, and after all three workflows succeed uses its narrow PR-write surface only to post one idempotent exact-head automation-approval audit comment.

`prepare-merge-authorization-read-only` then receives only `contents: read`, `pull-requests: read`, `checks: read`, and `actions: read`. It independently re-proves live `main`, live `generated`, the immutable candidate ref/head, exact PR and bot-commit topology, the three selected canonical workflow runs including run attempts/check-suite identities, and exactly the five successful GitHub-Actions checks. It constructs deterministic certificate and subject bytes bound to the exact lease/candidate/source/PR/run/check tuple. The certificate explicitly is not a freshness proof and cannot authorize merge without terminal independent revalidation.

`attest-merge-authorization-write-only` receives only `contents: read`, `id-token: write`, and `attestations: write`. It re-proves the exact short-lived mutation lease with a minimum remaining lifetime covering its hard timeout, digest-checks the downloaded certificate and subject against the preparer outputs, and runs only the canonical pinned `actions/attest` signer. It has no checkout, setup-Python, repository-authored Python, GitHub API mutation, content-write, PR, check, or Actions-write authority.

`merge-readme-only-terminal-write` re-proves the lease and receives `actions: read`, `contents: write`, `pull-requests: read`, `checks: read`, and `attestations: read` only; Actions authority is read-only and is used only to live-reprove the trusted Capability Admission run. It independently revalidates the exact PR head, `main` base, `generated` snapshot, immutable candidate ref, bot commit/content digest, one-file README diff, and five required checks; reconstructs the exact expected certificate subject; re-fetches the workflow runs represented by the certificate; and executes a literal `gh attestation verify` constrained to my repository, the Merge Authorization Certificate predicate type, the Spotlight signer workflow, exact source/signer digest, `refs/heads/main`, and hosted runners. The verified predicate must equal the downloaded certificate. Immediately before the expected-head merge it rechecks `main`, `generated`, and the candidate ref. After a successful merge it uses matching-ref cardinality to accept zero or one exact consumed candidate ref, verifies any remaining exact ref still targets the reviewed head, deletes it, and proves absence. Candidate refs are never moved.
...
| Spotlight merge proceeds on stale/unbound approval state | deterministic Merge Authorization Certificate after independent live revalidation | isolated lease-bound signer + terminal certificate verification plus independent fresh root/ref/check revalidation | GitHub attestation/control-plane compromise remains out of scope |
| Spotlight certificate signer mutates repository or approves checks | signer lacks content-write/PR/check/Actions-write authority | exact shell/action identity + short-lived lease | pinned signer/platform compromise remains in TCB |
...
- `attest-merge-authorization-write-only` intentionally holds OIDC/attestation authority; safety depends on the exact short-lived lease, digest-checked deterministic certificate/subject, canonical pinned signer, and the absence of repository/PR/check/Actions mutation authority.
- `merge-readme-only-terminal-write` intentionally holds `contents: write` plus read-only Actions/check/attestation authority; safety depends on independent live PR/ref/check and trusted-admission-run revalidation, exact certificate/subject equality, constrained cryptographic attestation verification, final root/ref freshness checks, expected-head merge semantics, and the server-side no-bypass Main ruleset.
...
12. confirm `stage-publication-read-only` remains read-only, publication has no signing authority, and the exact ten-job Spotlight graph keeps planning → lease mint → reductive reconciliation plus source-epoch mutation budget → diagnostic-only quarantine/proposal → approval → read-only Merge Authorization Certificate preparation → isolated certificate signing → terminal certificate-verifying merge authority separated;
...
14. confirm a real production `Update profile stats` run succeeds after production-path changes, including lease mint and all writer lease proofs, and a real changed Spotlight transaction exercises approval → MAC preparation → MAC attestation → terminal cryptographic verification → expected-head merge → candidate cleanup;
