...
For this migration, the checked-in target is the final seven-context set above while the live comparison temporarily accepts only two exact states: the legacy six contexts without `trusted-governed-bot-review`, or the final seven contexts with it. No partial, substituted, or extra context set is accepted. After the live ruleset reaches the seven-context state and readback is verified, this temporary two-state allowance is removed.
