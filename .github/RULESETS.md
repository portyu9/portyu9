...
This sentinel is detection only. Deliberate remediation is implemented separately by `.github/workflows/ruleset-reconciler.yml`, which is manual-dispatch only, serialized with a non-cancellable administration write queue, and bound to one exact reviewed repository/ruleset/endpoint/method/predecessor/successor transition. It uses the dedicated `ruleset-admin-identity` environment and repository-scoped GitHub App bootstrap secrets to mint a short-lived Administration-only installation token; it must never use `PORTYU9_BOT_REVIEW_TOKEN`.
