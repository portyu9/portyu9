...
This sentinel is detection only. Deliberate remediation is implemented separately by `.github/workflows/ruleset-reconciler.yml`. The reconciler wakes automatically on every accepted `main` push and retains `workflow_dispatch` only as a recovery path. Its read-only plan accepts only the exact reviewed predecessor or successor and exports that classification; the serialized, non-cancellable Administration job is entered only for the exact predecessor. When the exact successor is already live, ordinary `main` pushes stop before any admin secret is read, App token is minted, or ruleset write is attempted. The workflow remains bound to one exact reviewed repository/ruleset/endpoint/method/predecessor/successor transition, uses the dedicated `ruleset-admin-identity` environment and repository-scoped GitHub App bootstrap secrets to mint a short-lived Administration-only installation token, and must never use `PORTYU9_BOT_REVIEW_TOKEN`.
