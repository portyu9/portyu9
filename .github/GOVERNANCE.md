...
After deterministic reconciliation, the controller self-dispatches the existing workflow on the immutable candidate ref only so GitHub attributes the validation jobs to that exact head. Those jobs never checkout or execute candidate repository code. `validation_bind` checks out **static trusted `main`**, proves that checkout equals the bound base SHA, reconstructs only the exact four-file reconciled candidate from Git blob bytes as data, and executes trusted-main Dependabot validators against that temporary data copy. The `validate-contracts` and `integration-pinned-upstream` jobs likewise execute only static trusted `main`; the exact four-file closure proves the candidate cannot alter those executable validation surfaces. Dependency Review executes from static trusted `main` while comparing the exact bound base/head SHAs. CodeQL alone intentionally runs on the candidate head because the admitted dependency change is the CodeQL Action executable identity itself. Merge remains impossible until `validate-contracts`, `integration-pinned-upstream`, `dependency-review`, `trusted-capability-admission`, `analyze-actions`, and `analyze-python` are each one exact successful GitHub Actions check on that head. Immediately before mutation, the controller re-fetches the PR, current `main`, and candidate ref and then submits a merge request bound to the exact admitted head SHA. It has no `checks: write`, `security-events: write`, OIDC, attestation, or ruleset-bypass authority.
