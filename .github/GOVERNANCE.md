...
After deterministic reconciliation, the controller uses only the existing protected validation surfaces. CodeQL runs on the exact candidate head; Profile Quality receives a read-only workflow dispatch on that head; Dependency Review receives an **exact-head controller dispatch** binding the current `main` SHA and candidate SHA and retains `contents: read` only. Merge remains impossible until `validate-contracts`, `integration-pinned-upstream`, `dependency-review`, `trusted-capability-admission`, `analyze-actions`, and `analyze-python` are each one exact successful GitHub Actions check on that head. Immediately before mutation, the controller re-fetches the PR, current `main`, and candidate ref and then submits a merge request bound to the exact admitted head SHA. It has no `checks: write`, `security-events: write`, OIDC, attestation, or ruleset-bypass authority.
...
The same read-only workflow additionally exposes the reviewed **exact-head controller dispatch** used only by the delegated Dependabot CodeQL controller. That dispatch requires explicit base/head SHA inputs, proves the workflow is executing the requested head while `main` is still the requested base, and supplies those exact refs to Dependency Review. The vulnerability gate intentionally does not become a hidden **license policy** and does not authorize executable-identity changes by itself. Dependabot updates outside the delegated CodeQL class remain **never auto-merged**.
